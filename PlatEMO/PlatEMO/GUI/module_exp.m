...
%--------------------------------------------------------------------------
    ...
    methods(Access = ?GUI)
        ...
        function obj = module_exp(GUI)
            ...
            obj.app.labelA(9)  = GUI.APP(17,[1 2],uilabel(obj.app.grid(1),'Text','Number of results (>=2 for curves)','FontColor',[.15 .6 .2],'FontWeight','bold','Tooltip','Number of populations saved in each run. Set this to 2 or higher to record convergence curves such as IGD.'));
            obj.app.editA(2)   = GUI.APP(17,3,uieditfield(obj.app.grid(1),'numeric','Value',10,'limits',[1 inf],'RoundFractionalValues','on','Tooltip','Number of populations saved in each run. Values below 2 only keep the final population and produce a single-point IGD curve.','ValueChangedFcn',@(~,~)obj.UpdateResultSampleHint()));
            obj.UpdateResultSampleHint();
    ...
    methods(Access = private)
        ...
        function cb_start(obj,~,~)
            ...
            elseif strcmp(obj.app.buttonC(1).Text,'Continue')
                ...
                if obj.app.editA(2).Value < 2
                    warning('module\_exp:SingleSnapshot','Only one population will be saved per run. Increase "Number of results" to record IGD/HV curves.');
                end
                for p = 1 : length(PRO)
                    for a = 1 : length(ALG)
    ...
    methods(Access = private)
        function UpdateResultSampleHint(obj)
        %UpdateResultSampleHint - Highlight when too few populations are saved.

            if obj.app.editA(2).Value < 2
                obj.app.labelA(9).FontColor = [.75 .2 .2];
                obj.app.labelA(9).Text      = 'Number of results (only final population will be saved)';
            else
                obj.app.labelA(9).FontColor = [.15 .6 .2];
                obj.app.labelA(9).Text      = 'Number of results (>=2 for curves)';
            end
        end
    end
end
